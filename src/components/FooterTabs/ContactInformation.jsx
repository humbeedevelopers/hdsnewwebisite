...
return (
    <>
      <div className="w-full grid grid-cols-1 md:grid-cols-3 gap-10 py-10 px-5">
        {branch_details.map((object, index) => (
          <div key={index} className="flex flex-col items-center text-center gap-3">
            <h3 className="text-2xl md:text-3xl font-thin border-b border-primary/30 pb-2">{object.city}</h3>
            <p className="font-light text-sm md:text-base px-4 md:w-3/5">{object.address}</p>
            <a href={`tel:${object.contact}`} className="font-bold hover:text-primary transition-colors">
              {object.contact}
            </a>
          </div>
        ))}
